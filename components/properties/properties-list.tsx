...
function PropertyCard({ property, userRole }: { property: Property; userRole: UserRole }) {
  const router = useRouter();
  const canEdit = canCreateContent(userRole);
  const canArchive = canDeleteContent(userRole, property.createdBy);
...
export function PropertiesList({ properties, totalPages, currentPage, userRole }: PropertiesListProps) {
  return (
    <div className="space-y-6">
      {/* Properties Grid */}
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {properties.map((property) => (
          <PropertyCard key={property.id} property={property} userRole={userRole} />
        ))}
      </div>
