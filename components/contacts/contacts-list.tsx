...
function ContactCard({ client, userRole }: { client: Client; userRole: UserRole }) {
  const router = useRouter();
  const canEdit = canCreateContent(userRole);
  const canDelete = canDeleteContent(userRole, client.createdBy);
...
export function ContactsList({ clients, totalPages, currentPage, userRole }: ContactsListProps) {
  return (
    <div className="space-y-6">
      {/* Contacts Grid */}
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {clients.map((client) => (
          <ContactCard key={client.id} client={client} userRole={userRole} />
        ))}
      </div>
